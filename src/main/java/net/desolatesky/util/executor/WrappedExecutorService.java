...
public final class WrappedExecutorService implements ExecutorService {

    private final ExecutorService internal;

    public WrappedExecutorService(ExecutorService internal) {
    ...
    public @NotNull <T> List<Future<T>> invokeAll(@NotNull Collection<? extends Callable<T>> tasks) throws InterruptedException {
        return this.internal.invokeAll(tasks.stream().map(WrappedExecutorService::wrap).toList());
    }
    ...
    public @NotNull <T> List<Future<T>> invokeAll(@NotNull Collection<? extends Callable<T>> tasks, long timeout, @NotNull TimeUnit unit) throws InterruptedException {
        return this.internal.invokeAll(tasks.stream().map(WrappedExecutorService::wrap).toList(), timeout, unit);
    }

    @Override
    public @NotNull <T> T invokeAny(@NotNull Collection<? extends Callable<T>> tasks) throws InterruptedException, ExecutionException {
        return this.internal.invokeAny(tasks.stream().map(WrappedExecutorService::wrap).toList());
    }

    @Override
    public <T> T invokeAny(@NotNull Collection<? extends Callable<T>> tasks, long timeout, @NotNull TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
        return this.internal.invokeAny(tasks.stream().map(WrappedExecutorService::wrap).toList(), timeout, unit);
    }
