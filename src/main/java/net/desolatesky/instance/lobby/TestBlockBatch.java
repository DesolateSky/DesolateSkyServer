...
public class TestBlockBatch implements Batch<Runnable> {
    ...
    public TestBlockBatch() {
        this(new BatchOption());
    }

    public TestBlockBatch(BatchOption options) {
        this(options, true);
    }

    private TestBlockBatch(BatchOption options, boolean ready) {
     ...
     */
    @Override
    public TestBlockBatch apply(@NotNull Instance instance, @Nullable Runnable callback) {
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    public TestBlockBatch unsafeApply(@NotNull Instance instance, @Nullable Runnable callback) {
     ...
     * @return The inverse of this batch, if inverse is enabled in the {@link BatchOption}
     */
    protected TestBlockBatch apply(@NotNull Instance instance, @Nullable Runnable callback, boolean safeCallback) {
        if (!this.options.isUnsafeApply()) {
            System.out.println("Waiting");
            this.awaitReady();
        }
        System.out.println("Ready: " + this.isReady());

        final TestBlockBatch inverse = this.options.shouldCalculateInverse() ? new TestBlockBatch(inverseOption) : null;
        synchronized (chunkBatchesMap) {
            AtomicInteger counter = new AtomicInteger();
            Set<Chunk> updated = ConcurrentHashMap.newKeySet();
            for (var entry : Long2ObjectMaps.fastIterable(chunkBatchesMap)) {
                System.out.println("Applying chunk batch: " + entry.getLongKey());
                ...
                ChunkBatch chunkInverse = batch.apply(instance, chunkX, chunkZ, c -> {
                    System.out.println("Applying inverse");
