...
import com.fisherl.desolatesky.block.behavior.TickBehavior;
...
public final class IslandCoreTickBehavior implements TickBehavior {

    private static final Region BLOCK_REGION = Region.square(new Vec(0.5, 0, 0.5), 0.5, 0, 1);

    private final double generateItemChance;

    public IslandCoreTickBehavior(double generateItemChance) {
    ...
    public void onTick(DSWorld world, Point pos, Block block, Key blockId) {
        ...
        final Pair<Key, Integer> loot = lootTable.roll(randomGenerator);
        final ItemDefinition itemDefinition = itemFactory.getItemDefinition(loot.first().key()).orElse(null);
        if (itemDefinition == null) {
            return;
        }
        final ItemStack itemStack = itemDefinition.defaultItemStack().withAmount(loot.second());
    ...
    private static void spawnEntity(DSWorld world, Point pos, Block block, Key blockId) {
        ...
        entityFactory.createEntity(entityType, islandWorld.island(), e -> {
